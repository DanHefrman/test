...
World's simplest browser-based utility for printf-formatting text. Load your text in the input form on the left, specify the printf format in options below, and you'll instantly get printf-formatted text in the output area on the right. Powerful, free, and fast. Load text – get printf'ed text. Created by developers from team broowserools.
...
<span class="data-type">printf text</span> <span class="status"></span>
...
<span class="option-group-legend">Printf Format</span>

Printf format <span class="option-details">Set the format used by the printf() function here.</span>

<span class="primary">Text printf tool</span> <span class="secondary">What is a text printf?</span>

With this tool, you can format text according to the format specified to printf function. As printf and sprintf functions share the same format and logic, we actually use sprintf function internally to perform formatting. You can specify the format in the options above. The format can contain one or more specifiers that consist of a percentage character followed by the format code and can include optional flags. Flags are width, precision, and length. You can find documentation of all the format codes and flags in "sprintf" function's (or "printf" function's) manual page. Textabulous!

<span class="primary">Text printf examples</span> <span class="secondary">Click to use</span>

Scientific Notation

In this example, we set the sprintf format to "%e %s", which means – print any number in scientific format (%e) followed by all remaining text (%s).

245 apples 64 bytes 8 rabbits 556 cows 21 galaxies

2.45e+2 apples 6.4e+1 bytes 8e+0 rabbits 5.56e+2 cows 2.1e+1 galaxies
...
Right Align Flower Names

In this example, we format the input text, which is a list of flower names, using the "%15s" format. The % character indicates the beginning of a specifier, number 15 specifies the length of the line, the symbol s denotes text type. When this format is applied to all text lines, spaces get added on the left so that every line was 15 characters in length.

Rose Jasmine Lily Daffodil Lotus Tulip Sunflower Petunia
...
In this example, we format a set of decimal numbers with the specified precision. To do this, we use the "%.3f" specifier, where the number 3 indicates how many digits to leave after the decimal point.

30.017377 142.0105 3.033445929 24.15 7.37785 7.7 8.44393331 39.3152 0.234325

30.017 142.011 3.033 24.150 7.378 7.700 8.444 39.315 0.234
...
<span class="muted"><span class="green">https://</span>onlinetexttools.com/printf-text</span><span class="underline">?input</span>=245%20apples%0A64%20bytes%0A8%20rabbits%0A556%20cows%0A21%20galaxies&format=%25e%20%25s
