...
World"s simplest browser-based utility for checking if text is a palindrome. Load your text in the input form on the left and it"ll instantly get tested for its palindromeness. Powerful, free, and fast. Load plain text – check for a palindrome. Created by developers from team broowserools.
...
<span class="data-type">palindrome status</span> <span class="status"></span>
...
Text palindrome checker options

<span class="option-group-legend">Several Palindromes</span>

Line-by-line Mode <span class="option-details">Check multiple text lines at once.</span>

<span class="primary">Text palindrome checker tool</span> <span class="secondary">What is a text palindrome checker?</span>

With this tool you can test if your text is a palindrome. That is, the tool checks whether the given text is spelled equally from left to right and from right to left. If so, then in the output you get true, if not - false. To work with several text lines, select the line-by-line mode, then each test result will be next to the corresponding text row. Textabulous!

<span class="primary">Text palindrome checker examples</span> <span class="secondary">Click to use</span>

Palindromic Sentence

This example checks whether the sentence is read equally from both sides. It does and so it returns true.
...
In this example, we simultaneously check nine lines to see if they are palindromes. The line-by-line mode displays the results for each line individually.
...
<span class="muted"><span class="green">https://</span>onlinetexttools.com/check-text-palindrome</span><span class="underline">?input</span>=step%20on%20no%20pets
