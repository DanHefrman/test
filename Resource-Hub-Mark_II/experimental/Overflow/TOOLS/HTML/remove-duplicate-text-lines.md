[![onlinetexttools logo](images/logo-onlinetexttools.png)](index.html)
...
World's simplest browser-based utility for removing repeated lines from text. Load your text in the input form on the left and you'll instantly get text with no duplicate lines in the output area. Powerful, free, and fast. Load text lines – get unique text lines. Created by developers from team broowserools.
...
<span class="data-type">no duplicate lines</span> <span class="status"></span>
...
Duplicate text line remover options

<span class="option-group-legend">Operation Mode</span>

Remove All Duplicate Lines <span class="option-details">If this option is selected, then all repeated lines across entire text are removed.</span>

Remove Consecutive Duplicate Lines <span class="option-details">If this option is selected, then only consecutive repeated lines are removed.</span>

<span class="option-group-legend">Newlines, Tabs and Spaces</span>

Preserve All Newlines <span class="option-details">Leave all empty lines in the output.</span>

Filter All Newlines <span class="option-details">Process newlines as regular lines.</span>

Delete All Newlines <span class="option-details">Before filtering uniques, remove all newlines.</span>

Trim Text Lines <span class="option-details">Before filtering uniques, remove tabs and spaces from the beginning and end of all lines.</span>

<span class="option-group-legend">Sort Lines</span>

Sort the Output Lines <span class="option-details">After removing the duplicates, sort the unique lines.</span>

<span class="primary">Duplicate text line remover tool</span> <span class="secondary">What is a duplicate text line remover?</span>

With this tool you can remove repeated text lines from any text. It offers two different processing modes for doing this operation. The first mode removes all duplicate lines across the entire text. The second mode removes only the duplicate lines that are consecutive. You can further refine these operations by adjusting five different options. The first option preserves all newlines. What this means is that newlines get passed through and never get removed. This is useful if you're working with paragraphs of text and want to preserve the logical separation that newlines provide between the paragraphs. The second option filters all newlines. What this means is that all newlines are treated as regular lines and are checked for uniqueness. The third option deletes all newlines from the input and only non-blank lines are left. The fourth option trims lines before processing them. What this means is that all whitespace symbols (such as tabs and newlines) at the beginning of every text line are removed prior to continuing with finding unique lines. The fifth option allows you to sort the lines after they have been filtered. Textabulous!

<span class="primary">Duplicate text line remover examples</span> <span class="secondary">Click to use</span>

Favorite Movies List

This example loads a lengthy movie list in the input but there are a lot of duplicates. After processing the input, all repeated movie titles are removed and only unique text lines are left in the output. Also all accidental blank lines are removed and the output is alphabetically sorted.

The Great Gatsby Beautiful Mind Mr. Nobody Inception The Notebook The Butterfly Effect Beautiful Mind Fight Club The Notebook Inception The Shawshank Redemption Good Will Hunting Beautiful Mind

Beautiful Mind Fight Club Good Will Hunting Inception Mr. Nobody The Butterfly Effect The Great Gatsby The Notebook The Shawshank Redemption
...
In this example, we remove only the repeated duplicate lines from a summer ideas list. When performing this operation, the algorithm only looks at the previous line to find out if it's duplicated. Some ideas have whitespace symbols in front so we delete them as well.

Swimming Swimming Swimming Frozen Yogurt Frozen Yogurt Pick Wildflowers Disney Land Taco Truck Taco Truck Play Freeze Tag Swimming Frozen Yogurt

Swimming Frozen Yogurt Pick Wildflowers Disney Land Taco Truck Play Freeze Tag Swimming Frozen Yogurt
...
<span class="muted"><span class="green">https://</span>onlinetexttools.com/remove-duplicate-text-lines</span><span class="underline">?input</span>=The%20Great%20Gatsby%0ABeautiful%20Mind%0AMr.%20Nobody%0AInception%0AThe%20Notebook%0A%0AThe%20Butterfly%20Effect%0ABeautiful%20Mind%0AFight%20Club%0AThe%20Notebook%0A%0AInception%0AThe%20Shawshank%20Redemption%0AGood%20Will%20Hunting%0ABeautiful%20Mind&all-duplicates=true&delete-newlines=true&trim=false&sort=true
