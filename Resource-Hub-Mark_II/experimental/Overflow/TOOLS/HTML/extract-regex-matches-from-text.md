...
<span class="data-type">text that matches regex</span> <span class="status"></span>
...
Regex matches extractor options

<span class="option-group-legend">Regular Expression</span>

\[a-zA-Z0-9\]+

Regexp <span class="option-details">Enter the regular expression here.</span>

<span class="option-group-legend">Output Matches Separator</span>

Separator <span class="option-details">Enter the character that"s used for separating matched items.</span>

<span class="primary">Regex matches extractor tool</span> <span class="secondary">What is a regex matches extractor?</span>

With this tool, you can extract regular expression matches from the given text. You can specify the regexp in the options above and this tool will find and return all regexp matches. You can also change the character that separates all output matches. Textabulous!

<span class="primary">Regex matches extractor examples</span> <span class="secondary">Click to use</span>

Match and Extract Hex Colors

This example uses a regular expression that matches three or six character hexadecimal color codes. It finds all hex colors in the input text, returns a list of hex colors and separates them with a comma.

Each of the three pairs of digits in hex code \#RRGGBB is responsible for its color - red, green and blue. Numbers can take the values of digits from 0 to 9 and letters from A to F. If all numbers are maximal, then the result is white color - \#FFFFFF, if the numbers are minimal, it turns out black - \#000000. If you change each pair, you get \#FF0000 - the brightest red, \#00FF00 - the brightest green and \#0000FF - the brightest blue. Accordingly, \#00FFFF is cyan, \#FF00FF is magenta and \#FFFF00 is yellow.
...
In this example, we use a regex that matches one more characters between two numbers. It extracts a fragment of text that begins and ends with one or more digits.
...
1903 with her husband, Pierre Curie, and Henri Becquerel. Curie is also the only woman to have won multiple Nobel Prizes; in 1911
...
In this example, we enter a regexp to find prices in the given text. This regular expression takes into account various formats for displaying prices such as currency sign and cents separator that can be a dot or a comma.

Don't miss the sale of this season! Discounts on electronic goods up to 75%. You can purchase any TV from $300 or phones from €100. Buying a laptop from $499.99, get a 128GB USB flash drive as a gift.
...
<span class="muted"><span class="green">https://</span>onlinetexttools.com/extract-regex-matches-from-text</span><span class="underline">?input</span>=Each%20of%20the%20three%20pairs%20of%20digits%20in%20hex%20code%20%23RRGGBB%20is%20responsible%20for%20its%20color%20-%20red%2C%20green%20and%20blue.%20Numbers%20can%20take%20the%20values%20of%20digits%20from%200%20to%209%20and%20letters%20from%20A%20to%20F.%20If%20all%20numbers%20are%20maximal%2C%20then%20the%20result%20is%20white%20color%20-%20%23FFFFFF%2C%20if%20the%20numbers%20are%20minimal%2C%20it%20turns%20out%20black%20-%20%23000000.%20If%20you%20change%20each%20pair%2C%20you%20get%20%23FF0000%20-%20the%20brightest%20red%2C%20%2300FF00%20-%20the%20brightest%20green%20and%20%230000FF%20-%20the%20brightest%20blue.%20Accordingly%2C%20%2300FFFF%20is%20cyan%2C%20%23FF00FF%20is%20magenta%20and%20%23FFFF00%20is%20yellow.&regex=%2F%5C%23%28%5Ba-fA-F%5D%7C%5B0-9%5D%29%7B3%2C6%7D%2Fg&separator=%2C%20
