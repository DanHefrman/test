...
World's simplest browser-based utility for converting text to rot13. Load your text in the input form on the left and you'll instantly get rot13 in the output area. Powerful, free, and fast. Load text – get rot13(text). Created by developers from team broowserools.
...
<span class="data-type">rot13 text</span> <span class="status"></span>
...
<span class="primary">Rot13 converter tool</span> <span class="secondary">What is a rot13 converter?</span>

With this tool, you can encode or decode the given text to and from ROT13 encoding. It replaces each letter in the text by another letter that's 13 alphabetic positions away from it. The first letter of the alphabet (a) is replaced with the thirteenth letter (m), the second (b) with the fourteenth (n), and so on. Once the count goes past the last letter (z), it starts at the first letter of alphabet again (a). Also a fun fact – because the English alphabet has 26 letters and 13+13=26, this algorithm is inverse of itself (applying it twice to the text, we get the same text). Textabulous!

<span class="primary">Rot13 converter examples</span> <span class="secondary">Click to use</span>

Hide a Message Using ROT-13

In this example, we apply ROT-13 algorithm to a message about aliens to hide it from a casual reader.

An alien spacecraft within two spans.

Na nyvra fcnprpensg jvguva gjb fcnaf.

Reveal Text with ROT-13

In this example, we reveal a previously hidden text using ROT13 by applying the same algorithm to it (since it works both ways).

Qb abg ernq gur arkg fragrapr. Lbh yvggyr erory, V yvxr lbh!
...
<span class="muted"><span class="green">https://</span>onlinetexttools.com/rot13-text</span><span class="underline">?input</span>=An%20alien%20spacecraft%20within%20two%20spans.
