...
Text tabs to spaces converter options

<span class="option-group-legend">Spaces Settings</span>

Number of spaces <span class="option-details">Number of spaces to replace each tab symbol with.</span>

<span class="primary">Text tabs to spaces converter tool</span> <span class="secondary">What is a text tabs to spaces converter?</span>

With this tool you can replace tabs with spaces in the given text. By default, one space is put in place of one tab, however, you can change the number of spaces that are used for one tab in the options above. Textabulous!

<span class="primary">Text tabs to spaces converter examples</span> <span class="secondary">Click to use</span>

Each Tab to One Space

In this example, we remove all tabs in the text and put one space in place of each removed tab.
...
Format JS Code with Spaces

This example takes a recursive JavaScript function that"s indented by tabs and returns a retabbed version that uses 2 spaces for indentation.
...
In this example, we do something fun and set the number of spaces to zero. In this case, the tool removes all tabs and returns a merged phrase from Star Wars with no symbols separating words.
...
<span class="muted"><span class="green">https://</span>onlinetexttools.com/convert-tabs-to-spaces</span><span class="underline">?input</span>=To%09produce%09a%09single%09pound%09of%09honey%2C%09a%09single%09bee%09would%09have%09to%09visit%092%09million%09flowers.&spaces-per-tab=1
