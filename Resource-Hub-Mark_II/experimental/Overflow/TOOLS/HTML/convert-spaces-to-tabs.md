...
Text spaces to tabs converter
=============================

World"s simplest text tool
--------------------------

World"s simplest browser-based utility for converting spaces to tabs in text. Load your text in the input form on the left and you"ll instantly get text with tabs instead of spaces in the output area. Powerful, free, and fast. Load text – get tabs instead of spaces. Created by developers from team broowserools.
...
<span class="data-type">text with spaces</span> <span class="status"></span>
...
<span class="data-type">text with tabs</span> <span class="status"></span>
...
Text spaces to tabs converter options

<span class="option-group-legend">Replacement Options</span>

Replace Only Spaces <span class="option-details">Replace each space with exactly one tab.</span>

Replace Repeated Spaces <span class="option-details">Replace multiple repeated spaces with exactly one tab.</span>

Replace All Whitespaces <span class="option-details">Replace all spaces and newlines with exactly one tab.</span>

Replace Repeated Whitespaces <span class="option-details">Replace multiple repeated spaces, tabs, and newlines with a single tab.</span>

<span class="primary">Text spaces to tabs converter tool</span> <span class="secondary">What is a text spaces to tabs converter?</span>

With this tool you can replace spaces with tabs in the given text. In the options above, four replacement modes are offered for you. The first one replaces each space with a tab. For example, if you have five spaces one after another, you"ll get five tabs. The second one replaces repeated spaces with just a single tab. For example, if you have five repeated spaces, then all five will be replaced with a single tab. The third replaces all possible whitespaces (spaces, tabs, newlines, carriage return characters, vertical tabs, form feed characters) with a tab. For example, if you have a space followed by a newline, you"ll get two tabs. The fourth one replaces repeated whitespaces with a single tab. For example, if you have four whitespace characters, such as a space followed by a tab, followed by another space and a newline, you"ll get just one tab in place of all these four chars. Textabulous!

<span class="primary">Text spaces to tabs converter examples</span> <span class="secondary">Click to use</span>

Planet Sizes

This example replaces all spaces with tabs in scientific data about planet diameters. The input text is planet"s name, its size and measurement unit separated by spaces, and output is nicely formatted, aligned and easy to read data columns separated by tabs.

Planet Size Unit Mercury 4879 km Venus 12,104 km Earth 12,756 km Mars 6792 km Jupiter 142,984 km Saturn 120,536 km Uranus 51,118 km Neptune 49,528 km
...
In this example, we use text that contains many repeating spaces and we activate the replace-repeated-spaces option. As a result, all clusters of spaces get squished into a single tab and we get neat columns with information about how many moons each planet has.

Planet Moons Venus 0 Earth 1 Mars 2 Jupiter 79
...
This example activates the replace-all-whitespaces option. This option makes this tool match all characters that are considered whitespaces. These characters are spaces, tabs, newlines, carriage returns, and others. The input text in this example contains all these characters and they all get replaced by tabs.
...
In this example, we use text that has a lot of spaces, tabs, and newlines. We"ve enabled the replace-repeated-whitespaces function, which replaces clusters of whitespaces by a single tab. As a result, you get a sentence that has tabs instead of spaces between all words.
...
<span class="muted"><span class="green">https://</span>onlinetexttools.com/convert-spaces-to-tabs</span><span class="underline">?input</span>=Planet%20Size%20Unit%0AMercury%204879%20km%0AVenus%2012%2C104%20km%0AEarth%2012%2C756%20km%0AMars%206792%20km%0AJupiter%20142%2C984%20km%0ASaturn%20120%2C536%20km%0AUranus%2051%2C118%20km%0ANeptune%2049%2C528%20km&only-spaces=true
