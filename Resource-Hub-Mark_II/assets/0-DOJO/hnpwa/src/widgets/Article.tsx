...
export default factory(function Article({ properties }) {
  ...
  function renderArticle(article: ArticleItem) {
    ...
    const commentText =
      comments_count === 0 ? "discuss" : `${comments_count} comments`;
      ...
      <virtual>
        <h2 styles={{ width: undefined }}>
          {isComment ? (
            <Link
              to="comments"
              params={{ id: url.substr(8) }}
              classes={css.titleLink}
            >
              {title}
            </Link>
          ) : (
        ...
        <p classes={[css.details]}>
          <span key="points">{`${points || 0} points ${
            user ? "by " : ""
          }`}</span>
    ...
    return (
      <virtual>
        <h2
          ...
          classes={[css.titleShell, css.animated, css.animatedTitle]}
        />
  ...
  return (
    <article classes={[css.root]}>
      ...
      <div classes={[css.post]}>
        {item ? renderArticle(item) : renderArticleShell()}
      </div>
    </article>
  );
});
