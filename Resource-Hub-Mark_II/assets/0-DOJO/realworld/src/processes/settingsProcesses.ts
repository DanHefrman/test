...
const emailInputCommand = commandFactory<EmailPayload>(
  ({ payload: { email }, state }) => {
    state.settings.email = email;
  }
);

const passwordInputCommand = commandFactory<PasswordPayload>(
  ({ payload: { password }, state }) => {
    state.settings.password = password;
  }
);

const usernameInputCommand = commandFactory<UsernamePayload>(
  ({ payload: { username }, state }) => {
    state.settings.username = username;
  }
);

const bioInputCommand = commandFactory<BioPayload>(
  ({ payload: { bio }, state }) => {
    state.settings.bio = bio;
  }
);

const imageUrlInputCommand = commandFactory<ImagePayload>(
  ({ payload: { imageUrl }, state }) => {
...
export const updateUserSettingsProcess = createProcess("update-user-settings", [
  updateUserSettingsCommand,
]);
export const usernameInputProcess = createProcess("username-input", [
  usernameInputCommand,
]);
export const emailInputProcess = createProcess("email-input", [
  emailInputCommand,
]);
export const passwordInputProcess = createProcess("password-input", [
  passwordInputCommand,
]);
export const bioInputProcess = createProcess("bio-input", [bioInputCommand]);
export const imageUrlInputProcess = createProcess("image-url-input", [
  imageUrlInputCommand,
]);
