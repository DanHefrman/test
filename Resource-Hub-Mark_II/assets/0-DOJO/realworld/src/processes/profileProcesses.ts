...
const startGetProfileFeedCommand = commandFactory(
  ({ state, payload: { page, type } }) => {
    state.profile.feed = {
      isLoading: true,
      isLoaded: false,
      category: type,
...
const followUserCommand = commandFactory<FollowUserPayload>(
  async ({ state, payload: { username, following } }) => {
...
const getProfileFeedCommand = commandFactory<{
  username: string;
  type: string;
  page: number;
