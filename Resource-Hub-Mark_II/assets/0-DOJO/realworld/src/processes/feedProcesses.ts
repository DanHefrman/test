...
const startFetchingFeedCommand = commandFactory<FetchFeedPayload>(
  ({ state, payload: { type, filter, page } }) => {
    if (!state.feed) {
      state.feed = {};
    }
...
export const fetchFeedProcess = createProcess("fetch-feed", [
  startFetchingFeedCommand,
  fetchFeedCommand,
]);
export const clearFeedProcess = createProcess("clear-feed", [clearFeedCommand]);
export const favoriteFeedArticleProcess = createProcess("fav-feed-article", [
  favoriteFeedArticleCommand,
]);
