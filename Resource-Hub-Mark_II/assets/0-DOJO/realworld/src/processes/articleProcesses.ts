...
const startLoadingArticleCommand = commandFactory(
  ({ state, payload: { slug } }) => {
    state.article[slug] = {
      item: undefined,
      comments: [],
...
const loadArticleCommand = commandFactory<SlugPayload>(
  async ({ state, payload: { slug } }) => {
...
const loadCommentsCommand = commandFactory<SlugPayload>(
  async ({ state, payload: { slug } }) => {
    const token = state.session?.token;
...
const addCommentCommand = commandFactory<AddCommentPayload>(
  async ({ state, payload: { slug, newComment } }) => {
    const token = state.session?.token;
...
const deleteCommentCommand = commandFactory<DeleteCommentPayload>(
  async ({ state, payload: { slug, id } }) => {
    const token = state.session?.token;
...
const newCommentInputCommand = commandFactory<NewCommentPayload>(
  ({ state, payload: { newComment, slug } }) => {
    state.article[slug].newComment = newComment;
  }
);

const deleteArticleCommand = commandFactory<SlugPayload>(
  async ({ state, payload: { slug } }) => {
    const token = state.session?.token;
...
export const deleteCommentProcess = createProcess("delete-comment", [
  deleteCommentCommand,
]);
export const addCommentProcess = createProcess("add-comment", [
  addCommentCommand,
]);
export const newCommentInputProcess = createProcess("new-comment-input", [
  newCommentInputCommand,
]);
export const favoriteArticleProcess = createProcess("fav-article", [
  favoriteArticleCommand,
]);
export const followUserProcess = createProcess("follow-user", [
  followUserCommand,
]);
export const deleteArticleProcess = createProcess("delete-article", [
