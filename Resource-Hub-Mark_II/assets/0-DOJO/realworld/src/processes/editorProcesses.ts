...
const titleInputCommand = commandFactory<TitlePayload>(
  ({ state, payload: { title } }) => {
    state.editor.title = title;
  }
);

const descriptionInputCommand = commandFactory<DescriptionPayload>(
  ({ state, payload: { description } }) => {
    state.editor.description = description;
  }
);

const bodyInputCommand = commandFactory<BodyPayload>(
  ({ state, payload: { body } }) => {
    state.editor.body = body;
  }
);

const tagInputCommand = commandFactory<TagPayload>(
  ({ state, payload: { tag } }) => {
    state.editor.tag = tag;
  }
);

const addTagCommand = commandFactory<TagPayload>(
  ({ state, payload: { tag } }) => {
...
const removeTagCommand = commandFactory<TagPayload>(
  ({ state, payload: { tag } }) => {
...
const getArticleForEditorCommand = commandFactory<SlugPayload>(
...
export const titleInputProcess = createProcess("title-input", [
  titleInputCommand,
]);
export const descInputProcess = createProcess("desc-input", [
  descriptionInputCommand,
]);
...
export const addTagProcess = createProcess("add-tag", [
  addTagCommand,
  clearTagInputCommand,
]);
export const removeTagProcess = createProcess("remove-tag", [removeTagCommand]);
export const getEditorArticleProcess = createProcess("get-editor-article", [
  getArticleForEditorCommand,
]);
export const publishArticleProcess = createProcess("publish-article", [
  startPublishCommand,
  publishArticleCommand,
]);
export const clearEditorProcess = createProcess("clear-editor", [
  clearEditorCommand,
]);
